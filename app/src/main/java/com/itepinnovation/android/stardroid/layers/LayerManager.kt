...
class LayerManager(private val sharedPreferences: SharedPreferences) : OnSharedPreferenceChangeListener {
     ...
     */
    fun getObjectNamesMatchingPrefix(prefix: String): Set<SearchTermsProvider.SearchTerm> {
        val all: MutableSet<SearchTermsProvider.SearchTerm> = HashSet()
        for (layer in layers) {
            if (isLayerVisible(layer)) {
                for (query in layer.getObjectNamesMatchingPrefix(prefix)) {
                    val result = SearchTermsProvider.SearchTerm(query, layer.layerName)
                    all.add(result)
                }
            }
        }
